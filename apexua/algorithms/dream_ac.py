...
class dream(_algorithm):
        ...
        for i in range(self.N):  # Go through parameters
            ...
            if newN[i] == True:
                ...
                new_parameterset.append(
                    ...
                    + (1.0 + lambda_)
                    * gamma
                    * np.array(random_par_set1[i] - random_par_set2[i])
    ...
    def get_r_hat(self, parameter_array):
        ...
        if alreadyToNum > 3:
            parameter_array = parameter_array[
                :, int(np.floor(alreadyToNum / 2)) : alreadyToNum, :
            ]
        else:
    ...
    def sample(
        ...
        param_generator = (
            (curChain, list(startpoints[curChain]))
        ...
        while self.iter < self.repetitions:
            param_generator = (
                (curChain, self.get_new_proposal_vector(curChain, newN, c))
                for curChain in range(int(self.nChains))
            )
            for cChain, par, sim in self.repeat(param_generator):
                ...
                if metro_opt == 1:
                    ...
                    logMetropHastRatio = np.exp(
                        -0.5 * (-like + self.bestlike[cChain]) / (Sigma**2)
